...
Altered by [Alexander Breen](http://breen.io) to support tables that
contain only smaller headings, if all the smaller headings are nested
under just one outer heading.
...
class TocTreeprocessor(Treeprocessor):
    ...
    def run(self, doc):
        ...
        if len(toc_list_nested) == 1:
            toc_list_nested = toc_list_nested[0]['children']
